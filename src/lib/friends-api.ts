...
export const loadFriendsHome = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<FriendsHome> => {
    ...
    const meRows = await sql<ProfileRow>`
      select user_id, name, avatar_id, friend_code, level, rank_id, rank_percentile,
    ...
    const friendRows = await sql<ProfileRow>`
      select p.user_id, p.name, p.avatar_id, p.friend_code, p.level, p.rank_id, p.rank_percentile,
    ...
    const incomingRows = await sql<ProfileRow & { request_id: string }>`
      select r.id as request_id, p.user_id, p.name, p.avatar_id, p.friend_code, p.level, p.rank_id,
